...
export function RandomizerGame({
  client,
  rewards,
  visitedLocations,
}: RandomizerGameProps) {
  const slotdata = client.getSlotData();
  const nCrossLetters = Math.floor(
    slotdata.cross_letters_per_reward * rewards.nCrossLetterRewards,
  );
  const nRevealedClues =
    slotdata.n_starting_clues +
    Math.floor(slotdata.clues_per_reward * rewards.nClueRewards);
  const revealedLetterIndicies = new DefaultMap<string, number[]>(() => []);
  slotdata.cross_letters.slice(0, nCrossLetters).forEach((cl) => {
    revealedLetterIndicies
      .getOrCreate(clue_id_to_string(cl.clue_id))
      .push(cl.index);
  });
  ...
  return (
    <Box className="clues-container" p={2}>
      {slotdata.clues.map((clue, index) => {
        ...
        const isUncensored = isSolved || index < nRevealedClues;
        ...
        return (
          <ClueCard
            ...
            revealedLetters={lettersForClue}
          />
        );
      })}
    </Box>
